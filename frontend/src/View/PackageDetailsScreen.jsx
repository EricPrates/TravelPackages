...
export default function PackageDetailsScreen({ route }) {
    ...
    const validateMixed = () => {
        ...
        if (cash < 0 || miles < 0) return 'Valores negativos não são permitidos';
        if (cash === 0 && miles === 0) return 'Informe dinheiro ou milhas';
        if (cash > totalMoney) return `Máximo R$ ${totalMoney.toFixed(2)}`;
        if (miles > totalMiles) return `Máximo ${totalMiles.toLocaleString()} milhas`;
        return '';
    };
                ...
                <View style={styles.mixedPurchaseContainer}>
                    ...
                    <View style={styles.inputsContainer}>
                        <TextInput
                            ...
                            placeholder={`Dinheiro (máx R$ ${totalMoney.toFixed(2)})`}
                            placeholderTextColor="#94a3b8"
                            keyboardType="decimal-pad"
                            value={cashAmount}
                            onChangeText={(v) => handleMixedInputsChange(setCashAmount, v)}
                            editable={!isLoading}
                        />
                        <TextInput
                            style={[styles.input, mixedError && styles.inputError]}
                            placeholder={`Milhas (máx ${totalMiles.toLocaleString()})`}
                            placeholderTextColor="#94a3b8"
